...
namespace SW.Network
{
    public class IPUtils
    {
        ...
        public static IPEndPoint GetHostEndPoint(string host, int port)
        {
            ...
            {
#if TestIPv6
                Debug.Log("UdpSocket.GetHostEndPoint() TestIPv6: 如果已经是IP了，则封装成IPv6格式");
                ...
                if (!address.IsIPv6LinkLocal && !address.IsIPv6SiteLocal)
                {
                    ...
                    {
                        Debug.LogError("UdpSocket.GetHostEndPoint() TestIPv6，封装IPv6失败:" + tmp);
                    }
                }
            ...
            else
            {
                ...
                //if (Debug.EnableLog)
                //{
                //    Debug.Log("UdpSocket.GetHostEndPoint() Dns GetHostAddresses:");
                //    for (int i = 0; i < ips.Length; i++)
                //    {
                //        Debug.Log("[" + i + "] " + ips[i] + ", " + ips[i].AddressFamily);
                //    }
                //}
